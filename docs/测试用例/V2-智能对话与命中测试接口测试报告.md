...
| CHAT-06 | search_type 非法值 | `{"kb_id":"kb1", "query":"test", "search_type":"invalid"}` | **当前行为：不校验**。建议返回 `code: 400` |
...
| CHAT-11 | 全部文档 pending | KB 下有 3 篇文档，全部 status=pending | `code: 4002, msg: "知识库下 3 篇文档正在处理中..."` |
...
| HIT-13 | 文档 processing | status=processing | `code: 4002, msg: "文档「xxx」正在处理中..."` |
| HIT-14 | 文档 failed | status=failed | `code: 4002, msg: "文档「xxx」处理失败..."` |
...
> ⚠️ **当前阻塞**：两个接口都没有 `user=Depends(get_current_user)` 依赖，权限校验全部无法进行。

| 编号 | 场景 | 预期结果 |
|:--:|------|------|
| PERM-01 | 用户 A（组 G1 授权 KB1）对 KB2 调用 `/chat/send` | `code: 403` |
| PERM-02 | 用户 A 对 KB2 调用 `/test_retrieve` | `code: 403` |
| PERM-03 | 未分配用户组的用户调用 `/chat/send` | `code: 4001` |
| PERM-04 | 未分配用户组用户调用 `/test_retrieve` | `code: 4001` |
| PERM-05 | admin 用户访问任意 KB | `code: 0`（超管放行） |
| PERM-06 | 未登录调用 `/chat/send` | `code: 401` |
| PERM-07 | 未登录调用 `/test_retrieve` | `code: 401` |
