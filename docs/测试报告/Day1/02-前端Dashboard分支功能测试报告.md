...
| 3 | 角色选择（管理员/编辑员/普通用户） | ✅ | `el-radio-group`，三个选项 |
| 4 | 表单空值校验 | ✅ | 三项必填，rules 配置正确 |
| 5 | 登录按钮 loading 状态 | ✅ | `:loading="loading"` 防重复提交 |
| 6 | Mock 认证：admin / admin123 | ✅ | `password === username + '123'` |
| 7 | Mock 认证：editor / editor123 | ✅ | 同上 |
| 8 | Mock 认证：user / user123 | ✅ | 同上 |
| 9 | 错误密码拦截 | ✅ | 密码不匹配时 throw error，不跳转 |
| 10 | 管理员登录 → 跳转 `/dashboard` | ✅ | `redirectPath` 逻辑正确 |
| 11 | 编辑员登录 → 跳转 `/knowledge-bases` | ✅ | 同上 |
| 12 | 普通用户登录 → 跳转 `/chat` | ✅ | 同上 |
| 13 | Token 写入 localStorage | ✅ | key: `token` |
| 14 | 用户信息写入 localStorage | ✅ | key: `userInfo`，JSON 格式 |
| 15 | 登录成功 Toast 提示 | ✅ | `ElMessage.success('登录成功')` |
...
| 5 | **Mock 兜底** | ❌ | `dashboard.js` 无 try/catch Mock 降级 |
...
| # | 测试项 | 结果 | 说明 |
|:--:|------|:--:|------|
| 1 | 左侧固定导航（200px） | ✅ | `el-aside`，深色背景 #304156 |
| 2 | 顶部通栏 | ✅ | `el-header`，白色背景 |
| 3 | 右侧内容区 | ✅ | `el-main`，灰色背景 #f5f7fa |
| 4 | Logo 区域 | ✅ | 红色方块 + "智能RAG平台" |
| 5 | 菜单项按角色显示/隐藏 | ✅ | `canSeeMenu()` 函数 |
...
| 级别 | 编号 | 描述 | 位置 |
|:--:|:--:|------|------|
| 🟡 P1 | BUG-F01 | **Dashboard API 缺少 Mock 兜底**，后端不可用时显示"网络异常"toast | `src/api/dashboard.js` |
| 🟡 P1 | BUG-F02 | **`el-button type="text"` 已废弃**，应使用 `link` 属性 | `src/views/Layout.vue:52-53` |
| 🟡 P1 | BUG-F03 | **`request.js` 拦截器缺少 V2 错误码处理**（4001/4002/4003/403） | `src/utils/request.js` |
| 🟢 P2 | BUG-F04 | 已登录用户访问 `/login` 不自动跳转 | `src/router/index.js` |
| 🟢 P2 | BUG-F05 | `deleteUserApi` Mock 层做软删除，函数名暗示硬删除 | `src/api/users.js` |
| 🟢 P2 | BUG-F06 | 5 个文件为空：BrandingConfig.vue、branding.js、UserGroupManage.vue、DocManage.vue | `src/views/`、`src/stores/` |
| 🟢 P2 | BUG-F07 | Mock 数据页面刷新后重置 | `src/api/*.js` |
...
| 接口 | 前端文件 | 当前状态 |
|------|------|:--:|
| `POST /api/auth/login` | `src/api/auth.js` | Mock 兜底 |
